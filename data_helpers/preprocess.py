...
import string
import sys
import data_helpers.twokenize as twokenize
import csv
...
def main(csvfile):
    ...
    prccd_item_list = process(columns['tweet_text'])
    ...
    with open("%s_prccd.csv" % name, 'w') as f:
        # with codecs.open("%s_AIDR_prccd.csv" % name, 'wb', "utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["tweet_id", "tweet_text", "label"])
        rows = list(zip(columns['tweet_id'], prccd_item_list, columns['label']))
